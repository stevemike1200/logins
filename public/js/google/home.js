...
auth.onAuthStateChanged(user => {
	...
	if(user.email && user.phoneNumber) {
		...
		email5.innerHTML = `Logged in anonymously, you can link a valid email address or phone number and use it to get an invoice. This is optional if you prefer to remain anonymous`;
...
function sendEmail() {
	if(!localStorage.getItem('verify-cx')) {
		...
		var shortCutFunction = 'success';
		var msg = `
			Verification email has already been sent to <br>  ${auth.currentUser.email}
			<hr class="to-hr">
			Check the spam / junk folder.
		`;
		toastr.options = {
