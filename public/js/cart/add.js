...
function showNotify() {
    ...
    logz = JSON.parse(localStorage.getItem('banklogs'));

    if(JSON.parse(localStorage.getItem('banklogs')).length == 1) {
        for(var i = 0; i < logz.length; i++) {
            var shortCutFunction = 'success';
            var msg = `
                ${logz[0].account} with ${logz[0].balance} added to cart.
                <hr class="to-hr">
                Get 2 bank logs and enjoy a 10% discount.
            `;
            toastr.options = {
                closeButton: true,
                debug: false,
        ...
        for(var i = 1; i < logz.length; i++) {
            var shortCutFunction = 'success';
            var msg = `
                ${logz[1].account} with ${logz[1].balance} added to cart.
                <hr class="to-hr">
